...
if (nbmots-pastread) > imax
    c4 = fread(fidslice,imax,'single');
    pastread=pastread+imax;
else
    leftinline=nbmots-pastread;
    c4(1:leftinline) = fread(fidslice,leftinline,'single');
    empty=fread(fid,2,'single');
    stilltoread=imax-leftinline;
    c4(leftinline+1:imax) = fread(fidslice,stilltoread,'single');
